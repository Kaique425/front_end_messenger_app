    ...
    return (
        ...
        <div className='sectors-container'>
            <div className="sector-creation-title">
                ...
                <input className="sector-creation-input" type="text" placeholder="Digite o nome do Setor" onChange={(e) => setSectorNameInput(e.target.value)}/>
                <button className="sector-creation-button" onClick={() => createSector()} >Salvar</button>
            </div>

            <div className="sectors-visualization">
                <h1>Setores Cadastrados:</h1>
                {  sectors?.map( (sector) => (
                    <SectorItem key={sector.id} sector={sector} setSector={setSectors} sectors={sectors} />
                ))
                }   
            </div>
        </div>
