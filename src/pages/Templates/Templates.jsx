...
import './style.css';
import { KeyTemplateIcon } from '../../Icons/KeyTemplateIcon';
import { MarketingTemplateIcon } from '../../Icons/MarketingTemplateIcon';
import { MessageNotificationBell } from '../../Icons/MesssageNotificationBell';
import { cleanHeader, cleanTitle } from '../../utils/templateFieldCleaners';
...
const Templates = () => {
  ...
  const handleTemplateInfoChange = (e, type) => {
    if (type === 'header') {
      let cleanedHeader = cleanHeader(e.target.value);
      e.target.value = cleanedHeader;
    }

    if (type === 'title') {
      let cleanedTitle = cleanTitle(e.target.value);
      e.target.value = cleanedTitle;
    }
    ...
    setTemplateInfo((prevState) => ({
      ...
      [type]: {
        ...
        example: {
          ...
          [example_key_name]: ['Kaique HARDCODED'],
        },
      },
    }));
  };
  ...
  const handleTemplateCreation = async (templateInfo) => {
  ...
  useEffect(() => {
    console.log(`${templateInfo.title?.text}`);
    console.log(`${templateInfo.header?.text}`);
    console.log(`${templateInfo.body?.text}`);
    console.log(`${templateInfo.footer?.text}`);
    const teste1 = 'Bom dia {{1}} {{2}}';
    const pattern = /\{\{[^\}]+\}\}/g;
    const numberOfOcurrences = templateInfo?.body?.text.match(pattern);
  ...
  return (
    <div className='hsms-container max-w-[1113px]'>
        ...
        <div className='template-creation-container'>
          <div className='template-creation-inputs'>
            <div className='template-div template-name-div'>
              <label
                htmlFor='template-title-field'
                className='template-div-title'
              >
              ...
              <div className='template-field-input'>
                <input
                  type='text'
                  id='template-title-field'
                  maxLength={512}
                  required={true}
                  onChange={(e) => handleTemplateInfoChange(e, 'title')}
                />
                <span className='character-limit-span'>
            ...
            <div>
              <h3>Conteúdo</h3>
              <span className='template-content-desc'>
                Preencha as seções de cabeçalho, corpo e rodapé do seu modelo.
              </span>
            </div>
            <div className='template-main-inputs-div'>
              <div className='template-header-div'>
                <label
                  htmlFor='template-header-field'
                  className='template-div-title'
                >
                  <strong>Cabeçalho</strong>
                  <span className='template-optional-desc'>Opcional</span>
                </label>
                <div className='template-field-input'>
                  <input
                    ref={templateHeaderRef}
                    id='template-header-field'
                    onChange={(e) => handleTemplateInfoChange(e, 'header')}
                    type='text'
                    maxLength={60}
                  />
                  <span className='character-limit-span'>
                ...
                <button
                  onClick={() =>
                    AddTemplateVariable({
                      elementRef: templateHeaderRef,
                      variableQuantityLimit: 1,
                    })
                  }
                  className='template-add-variable-button'
                >
                  + Adicionar variável
                </button>
              </div>
              ...
              <div className='template-body-div'>
                ...
                <div>
                  <textarea
                    ref={templateBodyRef}
                    onChange={(e) => handleTemplateInfoChange(e, 'body')}
                ...
                <button
                  onClick={() =>
                    AddTemplateVariable({
                      elementRef: templateBodyRef,
                      variableQuantityLimit: null,
                    })
                  }
                  className='template-add-variable-button'
                >
                ...
                <span className='character-limit-span'>
                  Limite de caracteres 1024
                </span>
              </div>

              <div className='template-footer-div'>
                <label
                  htmlFor='template-footer-field'
                  className='template-div-title'
                >
                ...
                <div className='template-field-input'>
                  <input
                    id='template-footer-field'
                    type='text'
                    onChange={(e) => handleTemplateInfoChange(e, 'footer')}
                    maxLength={60}
                  />
                  <span className='character-limit-span'>
                    Limite de caracteres 60
          ...
          <div className='template-creation-preview'>
            <div className='template-div-title'>
            ...
            <div className='modal-template-buttons-container'>
              <button
                className='template-action-button cancel-button'
                onClick={() => handleCloseModal()}
              >
                Cancelar
              </button>
              <button
                className='template-action-button create-button'
                onClick={() => handleTemplateCreation(templateInfo)}
              >
                Enviar para Análise.
              </button>
            </div>
      ...
      <table className='hsm-table'>
        ...
        {hsms &&
          hsms.map((hsm) => (
            <tbody key={hsm.id}>
              <tr>
                <td data-cell='name' className='hsm-field-item'>
                  {hsm.name}
                </td>
                <td
                  data-cell='category'
                  className='hsm-field-item category-item '
                >
                  {hsm.category}
                </td>
                <td data-cel='status' className='hsm-field-item'>
                  {hsm.status === 'pending' ? (
                    ...
                    <div className='status disabled-status-div'>
                      Desabilitado
                    </div>
                  ) : (
                ...
                <td data-cell='language' className='hsm-field-item'>
                  {hsm.language_code}
                </td>
              </tr>
            </tbody>
