...
export const AttendanceItem = ({ attendanceItem, openAttandance }) => {
  return (
    <div
      ...
      key={attendanceItem.customer_phone_number}
      onClick={() => openAttandance(attendanceItem)}
    >
      <div className='attendance-infos'>
        <WhatsAppIcon />
        <div>
          <div>
            <strong>{attendanceItem.customer_name}</strong>
          </div>
          <div>{attendanceItem.customer_phone_number}</div>
          {attendanceItem.unread_messages_quantity !== 0 && (
            <span className='unread-message-quantity'>
              {attendanceItem.unread_messages_quantity}
            </span>
          )}
        </div>
      </div>
