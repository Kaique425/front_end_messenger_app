...
export const SectorItem = ({sector, setSector, sectors}) => {
    ...
    const deleteSector = async () => {
        ...
        console.log(`${response.status} FINALIZADO COM SUCESSO`)
    ...
    return (
        <div className="sector-item" >
            <div className="sector-id" ><strong>ID:</strong> {sector?.id}</div>
            <div className="sector-name" ><strong>Nome:</strong> {sector?.name}</div>
            <button className="sector-delete-button" onClick={() => deleteSector()} >Apagar</button>
        </div>
    )
}